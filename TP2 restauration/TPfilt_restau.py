...
def viewimage_color(im, normalise=True, MINI=0.0, MAXI=255.0):
    ...
    if platform.system() == 'Darwin':  # on est sous mac
        ...
        # Remplacer D:/... par le chemin de votre GIMP
        prephrase = 'start /B "D:/GIMP/bin/gimp-2.10.exe" -a '
        endphrase = ''
    else:
...
% 
    ...
    if ech == 0:  # plus proche voisin
        ...
        out = I00*(1.0-PoidsX)*(1.0-PoidsY)+I01*(1-PoidsX) * \
            PoidsY+I10*PoidsX*(1-PoidsY)+I11*PoidsX*PoidsY
    return out
...
def median_filter(im, typ=1, r=1, xy=None):
    ...
    for k in range(len(lx)):
        tab[k, :] = im[deby+ly[k]:deby+tty+ly[k],
                       debx+lx[k]:debx+ttx+lx[k]].reshape(-1)
    out = im.copy()
...
# on ne normalise pas pour garder l'image comme
viewimage_color(im, normalise=False)
# a l'origine
...
# le reshape est inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
...
viewimage(im)
# le reshape en inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
(histo, bins) = np.histogram(im.reshape((-1,)), np.arange(0, 256))
histo = histo/histo.sum()
histocum = histo.cumsum()
...
# unravel donne les index 2D a partir des index 1d renvoyes par argsort (axis=None)
...
viewimage(rotation(im, 45, ech=0, clip=False))  # plus proche voisin
viewimage(rotation(im, 45, ech=1, clip=False))  # transfo bilineaire

# %% Rotation 8 fois de 45deg par plus proche voisin
im2 = rotation(im, 45, clip=False)
for i in range(7):
    im2 = rotation(im2, 45, ech=0, clip=False)


viewimage(im2)

# %% Rotation 8 fois bilineaire
im3 = rotation(im, 45, ech=1, clip=False)
for i in range(7):
    im3 = rotation(im3, 45, ech=1, clip=False)

viewimage(im3)

# %% rotation avec un facteur de zoom inferieur a 1 (1/2)
im = skio.imread('images/carte_nb.tif')

im2 = rotation(im, 45, alpha=0.5, clip=False)
im3 = rotation(im, 45, alpha=1, clip=False)
viewimage(im2)
viewimage(im3)
...
kernel = get_gau_ker(100)
im_f = filtre_lineaire(im, kernel)
viewimage(im)
viewimage(im_f)
...
kernel = get_gau_ker(3)
im_f = filtre_lineaire(im_n, kernel)

im_f_median = median_filter(im_n, typ=1, r=5)

print(var_image(im_n, 6, 89, 77, 169))
print(var_image(im_f, 6, 89, 77, 169))
print(var_image(im_f_median, 6, 89, 77, 169))
# viewimage(im_n)
viewimage(im_f)
viewimage(im_f_median)
...
kernel = get_gau_ker(3)
im_f = filtre_lineaire(im, kernel)
im_f_median = median_filter(im, typ=1, r=1)
...
# Creation du masque
mask = get_gau_ker(25)
im_f = filtre_lineaire(im, mask)  # filtre applique a l'image

# image bruitée
im_noise = noise(im_f, 5)

im_nf = filtre_inverse(im_f, mask)
...
im_wiener1 = wiener(im_noise, np.eye(1), lamb=1)
...
# Image degradee
im_noise = noise(im, 30)


# Image restaurée par wiener
im_new = wiener_new(im_noise, np.eye(1))
